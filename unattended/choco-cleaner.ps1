...
# Minor logging
Write-Output "$(Get-Date) Choco-Cleaner STARTED"
...
if (Test-Path $ENV:ChocolateyInstall\bin\_processed.txt){
   Write-Host "  **  Deleting unnecessary Chocolatey _processed.txt file..." -Foreground Green
...
if ($ENV:ChocolateyInstall -Match $ENV:SystemDrive -and $ENV:SystemDrive -eq "C:")
    {
	 ...
	 Write-Output "$(Get-Date) Choco-Cleaner FINISHED and reclaimed ~ $FreedSpace KB!"
    } else {
	  Write-Host "Choco-Cleaner finished deleting unnecessary Chocolatey files!" -Foreground Magenta
	  Write-Output "$(Get-Date) Choco-Cleaner FINISHED"
	 }
